...
#### Есть класс Book. У Book есть поля (свойства книги)
...
Для каждого поля должны быть getters.
...
В классе Reader должен быть метод который рандомно определяет какую книгу будет читать пользователь:
<br>public Book choiceBook(Book[] books)

**choiceBook** 
- Принимает в себя массив книг и с помощью java.util.Random выбирает одну книгу и возвращает ее;
- В классе Reader должен быть метод void readBook(Book book) который принимает входным аргументом Book и выводит его в консоль с помощью переопределенного toString
...
**Сделать отдельный класс Library в котором будет выполняться программа**
<br>Работа программы выглядит следующим образом:
